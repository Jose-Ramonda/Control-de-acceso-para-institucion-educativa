    ...
    while(1) {
        ...
        if(xQueueReceive(uart_cola, (void *)&event, portMAX_DELAY)) {
            ...
            switch(event.type) {
                ...
                case UART_DATA: { 
                    int n = uart_read_bytes(UART_PORT, temp_rx_buf, event.size, pdMS_TO_TICKS(10));
                    
                    uint8_t checksum = 0;
                    
                    if( n >= 4 && temp_rx_buf[0] == 0xAA ){
                        ESP_LOGI(TAG, "RAW RX (%d bytes): [0x%02X 0x%02X 0x%02X 0x%02X...]", n, temp_rx_buf[0], temp_rx_buf[1], temp_rx_buf[2], temp_rx_buf[3]);
                        ...
                        case UART_CAM_PH_CMD:
                            composer(CMD_TAKE_PH,0,NULL,NULL);
                            break;
                        case UART_CAM_WFON_CMD:
                            composer(CMD_WIFI, temp_rx_buf[2], &temp_rx_buf[3], NULL);
                            break;
                        case UART_CAM_WFOFF_CMD:
                            composer(CMD_WIFI_FAIL,0,NULL,NULL);
                            break;
...
void cam_uart_send(uint8_t *trama, int len){
    ...
    ESP_LOGI(TAG,"ENVIANDO MENSAJE %d %d %d",exit[0],exit[1], exit[2]);
}